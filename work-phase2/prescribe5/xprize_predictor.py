...
class XPrizePredictor(object):
    ...
    def forecast(self, start_date, end_date, npis_df, remember_predictions=True) :
        ...
        truncated_df = self.df[self.df.Date < start_date]
        country_samples = self._create_country_samples(truncated_df, geos, False)
        smoothing = np.zeros((WINDOW_SIZE,))

        # For each requested geo
        geos = npis_df.GeoID.unique()
        for g in geos:
            cdf = truncated_df[truncated_df.GeoID == g]
            country = cdf.iloc[0].CountryName
            region = cdf.iloc[0].RegionName
            logging.info("Country=%s, Region=%s, g=%s",country,region,g)
            npis_gdf = npis_df[npis_df.GeoID == g]
            if len(cdf) == 0:
                ...
                npis_gdf = npis_gdf[(npis_df.Date >= geo_start_date) & (npis_df.Date <= end_date)]

                logging.info("XP: forecasting %s between %s until %s and ending on %s", g, geo_start_date, start_date, end_date)
                pred_new_cases = self._get_new_cases_preds(cdf, g, npis_gdf, country_samples)

            # Append forecast data to results to return
            last_smooth_new_cases = cdf.SmoothNewCases.iloc[-1]
            if remember_predictions :
                df_update = pd.DataFrame(columns=self.df.columns)
                df_update.Date = pd.date_range(geo_start_date, start_date - pd.offsets.Day(1))
                df_update = df_update.set_index('Date')
                last_confirmed_cases = cdf.ConfirmedCases.max()
                npis_gdf = npis_gdf.set_index('Date')
            for i, pred in enumerate(pred_new_cases):
                ...
                if current_date < start_date and remember_predictions:
                    df_update.loc[current_date,'Population'] = cdf.iloc[0].Population
                    df_update.loc[current_date,'ConfirmedDeaths'] = cdf.iloc[-1].ConfirmedDeaths
                    df_update.loc[current_date,'NewDeaths'] = cdf.iloc[-1].NewDeaths
                    df_update.loc[current_date,'CountryName'] = country
                    df_update.loc[current_date,'RegionName'] = region
                    df_update.loc[current_date,'NewCases'] = pred
                    last_confirmed_cases += pred
                    df_update.loc[current_date,'ConfirmedCases'] = int(last_confirmed_cases)
                    df_update.loc[current_date,'GeoID'] = g
                    df_update.loc[current_date,'ProportionInfected'] =  last_confirmed_cases / cdf.iloc[0].Population
                    df_update.loc[current_date,'SmoothNewDeaths'] =  cdf.iloc[-1].SmoothNewDeaths
                    df_update.loc[current_date,'DeathRatio'] =  cdf.iloc[-1].DeathRatio
                    if i < WINDOW_SIZE :
                        l = WINDOW_SIZE-i-1
                        smoothing[:l] = cdf.NewCases[cdf.shape[0] - l:]
                        smoothing[l:] = df_update.NewCases[:i+1]
                    else :
                        smoothing[:] = df_update.NewCases[df_update.shape[0] - WINDOW_SIZE:df_update.shape[0]]
                    current_smooth_new_cases = smoothing.mean()
                    df_update.loc[current_date,'SmoothNewCases'] = current_smooth_new_cases
                    cr = (current_smooth_new_cases - last_smooth_new_cases)/last_smooth_new_cases
                    if np.isnan(cr) or np.isinf(cr) :
                        cr = 0.0
                    df_update.loc[current_date,'CaseRatio'] = cr+1
                    last_smooth_new_cases = current_smooth_new_cases
                    df_update.loc[current_date,'PredictionRatio'] = df_update.loc[current_date,'CaseRatio'] / (1 - df_update.loc[current_date,'ProportionInfected'])
                    for x in NPI_COLUMNS :
                        df_update.loc[current_date,x] = npis_gdf.loc[current_date,x]

            if geo_start_date < start_date and remember_predictions:
                df_update = df_update.reset_index(drop=False)
                self.df = pd.concat([self.df,df_update])
                #self.df[self.df.GeoID == g].to_csv("geo_df.csv",index=False)
        # Update the df with the new data
        #if geo_start_date < start_date and remember_predictions:
        #    self.df = self.df.set_index(['CountryName','RegionName','Date']).sort_index().reset_index(drop=False)
    ...
    def predict(self,
        ...
        return self.forecast(start_date, end_date, npis_df)
    ...
    def _roll_out_predictions(predictor, initial_context_input, initial_action_input, future_action_sequence):
        ...
        for d in range(nb_roll_out_days):
            ...
            pred = predictor.predict([context_input.astype('float32'), action_input.astype('float32')])
            pred_output[d] = pred
