...
NPI_COLUMNS = undot(['C1_School.closing',
...
class XPrizePredictor(object):
    ...
    def __init__(self, path_to_model_weights, data_url=None, df=None):
    ...
    def predict(self,
        ...
        for g in geos:
            ...
            if len(cdf) == 0:
                ...
                geo_start_date = min(last_known_date - np.timedelta64(NB_LOOKBACK_DAYS, 'D'), start_date)
    ...
    def _get_new_cases_preds(self, c_df, g, npis_df, country_samples):
        ...
        # cnpis_df.rename(columns=lambda x: x.replace(' ', '.'), inplace=True)
