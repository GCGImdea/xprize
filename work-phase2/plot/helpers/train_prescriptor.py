...
from .utils import CASES_COL, IP_COLS, IP_MAX_VALUES, add_geo_id, NB_LOOKBACK_DAYS, POPULATION_FILE, policy
...
CUTOFF_DATE = '2021-01-25'  # '2020-07-31'
EVAL_START_DATE = '2021-01-26'
...
def loss_fn(inputs=None, y_pred_=None, testing=False):
    ...
    if inputs.shape[0] is not None:
        ...
        n_cases, _ = predict_future_cases(predictor, _prescribed_ips, context)
        # n_cases = n_cases[:, 0]
    ...
    def loss_fn_(y_true, y_pred):
        ...
        predictor_loss = tf.reduce_mean(((cases - n_cases)) ** 2) * 10
        ratio = 1  # 50. / 100000.  # ./1440  # 1440. / 2880913
        cases_loss = tf.reduce_mean(cases * ratio)
        stringency = tf.reduce_sum((prescribed_ips) / 24, axis=1)
        # print(cases.dtype, stringency.dtype)
        # agg_loss = K.sum((cases * ratio) + stringency, axis=1)
        agg_loss = tf.reduce_mean(stringency)  # K.sum(prescribed_ips, axis=1)  # K.mean(stringency)

        loss = 1 * agg_loss  # .1 * K.mean(agg_loss) * 1e-3
        # reg_loss = (1 + K.sum(n_cases)) / (agg_loss + 1e-5)
        # reg_loss = 32 * 1 * K.minimum(1000, reg_loss)

        reg_loss = tf.reduce_sum(agg_loss)
        if reg_loss == 0:
            loss *= 1e-5
...
def loss_fn2(inputs=None, y_pred_=None, testing=False):
    ...
    def loss_fn_(y_true, y_pred):
        ...
        stringency = tf.reduce_sum((prescribed_ips) / 24, axis=1)
        agg_loss = tf.reduce_mean(stringency)  # K.sum(prescribed_ips, axis=1)  # K.mean(stringency)
        # agg_loss = tf.cast(agg_loss, dtype=n_cases.dtype)
        n_cases = tf.cast(n_cases, dtype=agg_loss.dtype)
        n_cases = np.asarray(n_cases)

        reg_loss = (np.mean(n_cases) / (agg_loss + 1e-5))  # / (agg_loss*24*12 + 1e-5)

        loss = .01 * reg_loss
        # print(loss)
        # print(prescribed_ips)
        if loss >= 1000:
            loss = tf.reduce_sum((prescribed_ips - ip_max_values_arr) ** 2)
            loss = 1. * loss

        if np.random.rand() > 1.5:
            l1_reg_term = tf.reduce_sum(tf.abs(prescribed_ips / 24))
            if l1_reg_term > 0:
                l1_reg_term = tf.cast(l1_reg_term, dtype=agg_loss.dtype)
                loss += .001 * l1_reg_term
...
class Prescriptor:
    ...
    def __init__(self, df=None, data_path=None, path_to_cost_file=None, _predictor=None,
        ...
        self.df = df
        self.models_path = f"{root_path}/prescribe1/saved_models"
        ...
        for epoch in range(epochs):
            ...
            for step in range(0, len(training), 32):
                ...
                with tf.GradientTape() as tape2:
                    ...
                    grads2 = tape2.gradient(loss_value2, training_model.trainable_weights)
                    optimizer2.apply_gradients(zip(grads2, training_model.trainable_weights))
    ...
    def _lstm_roll_out_predictions(self, df, model, geo_id, initial_context_input, initial_action_input,
                                   initial_costs_input,
        ...
        for d, date in enumerate(pd.date_range(eval_start_date, eval_end_date)):
            ...
            if count_ == 1:
                ...
                prescribed_ips = policy(prescribed_ips, initial_costs_input)
