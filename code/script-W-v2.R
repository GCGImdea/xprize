...
provincial_regional_estimate_w_only <- function(countrycode = "ES",
  ...
  dt <- dt[, c("timestamp","region","reach","cases", "fatalities", "recentcases", "stillsick", "iso.3166.1.a2", "iso.3166.2", "cookie")]
  ...
  dt$ratio <- dt$cases/dt$reach    # remove outlier based on ratio cases/reach
  ...
  r_c <- r_r <- r_r_recent <- r_r_fatalities <- r_r_stillsick <-  
    I_c_p_w_country <- I_c_p_m_country <- I_c_recent_p_w_country <- I_c_recent_p_m_country <- 
    I_c_fatalities_p_m_country <- I_c_fatalities_p_w_country <- I_c_stillsick_p_m_country<- I_c_stillsick_p_w_country <- c()
  I_r_p_w_country <- I_r_p_m_country <- I_r_recent_p_w_country <- I_r_recent_p_m_country <- 
    I_r_fatalities_p_w_country <- I_r_fatalities_p_m_country <- I_r_stillsick_p_w_country <- I_r_stillsick_p_m_country <- c()
  p_w_country <- p_m_country <- recent_p_w_country <- recent_p_m_country <- 
    fatalities_p_w_country <- fatalities_p_m_country <- stillsick_p_w_country <- stillsick_p_m_country <- c()
  ...
  for (j in dates){
    ...
    if (province == T){
      ...
      fatalities_p_w_provs <- fatalities_p_m_provs <- rep(NA, length(provs))
      stillsick_p_w_provs <- stillsick_p_m_provs <- rep(NA, length(provs))
      for (i in seq_along(provs)) {
        ...
        if(nrow(dt_prov) != 0){
          ...
          fatalities_p_w_provs[i] <- ifelse(all(is.na(dt_prov$fatalities)), 
                                        0, sum(dt_prov$fatalities, na.rm = T)/sum(dt_prov$reach[!is.na(dt_prov$fatalities)]))  #should we use the whole sum or partial sum
          fatalities_p_m_provs[i] <- ifelse(all(is.na(dt_prov$fatalities)), 
                                        0, mean(dt_prov$fatalities/dt_prov$reach, na.rm = T))
          ...
          fatalities_p_w_provs[i] <- fatalities_p_m_provs[i] <- 0
          stillsick_p_w_provs[i] <- stillsick_p_m_provs[i] <- 0
        }
      ...
      dtestpropprovs <- data.frame(provincecode = provs,
                                   ...
                                   fatalities_p_w_provs = fatalities_p_w_provs,
                                   fatalities_p_m_provs = fatalities_p_m_provs,
    ...
    fatalities_p_w_regs_only <- fatalities_p_m_regs_only <- rep(NA, length(regions))
    stillsick_p_w_regs_only <- stillsick_p_m_regs_only <- rep(NA, length(regions))
    ...
    for (k in seq_along(regions)) {
      ...
      if(nrow(dt_reg) != 0){
        ...
        fatalities_p_w_regs_only[k] <- ifelse(all(is.na(dt_reg$fatalities)),
                                          NA, sum(dt_reg$fatalities, na.rm = T)/sum(dt_reg$reach[!is.na(dt_reg$fatalities)]))  # should we use partial sum?
        fatalities_p_m_regs_only[k] <- ifelse(all(is.na(dt_reg$fatalities)),
                                          NA, mean(dt_reg$fatalities/dt_reg$reach, na.rm = T))
        ...
        fatalities_p_w_regs_only[k] <- fatalities_p_m_regs_only[k] <- 0
        stillsick_p_w_regs_only[k] <- stillsick_p_m_regs_only[k] <- 0
      }
    ...
    dtestpropregs <- data.frame(regioncode = regions,
                                ...
                                fatalities_p_w_regs_only = fatalities_p_w_regs_only,
                                fatalities_p_m_regs_only = fatalities_p_m_regs_only,
    ...
    if(province == T){
      ...
      fatalities_p_w_regs_rhs <- fatalities_p_m_regs_rhs <- rep(NA, length(uregions))
      stillsick_p_w_regs_rhs <- stillsick_p_m_regs_rhs <- rep(NA, length(uregions))
      ...
      for (l in seq_along(uregions)) {
        ...
        fatalities_p_w_regs_rhs[l] <- sum(weightreg2 * dt_est_reg2$fatalities_p_w_provs)
        fatalities_p_m_regs_rhs[l] <- sum(weightreg2 * dt_est_reg2$fatalities_p_m_provs)
      ...
      dt_regs_rhs <- data.frame(regioncode = unique(dt_est_prov_reg$regioncode),
                                ...
                                fatalities_p_w_regs_rhs = fatalities_p_w_regs_rhs,
                                fatalities_p_m_regs_rhs = fatalities_p_m_regs_rhs,
      ...
      dt_est_prov_reg$fatalities_p_w_regs <- ((dt_est_prov_reg$sumreach_regs/(dt_est_prov_reg$sumreach_regs+dt_est_prov_reg$sumreach_regs_rhs2)) *
                                            dt_est_prov_reg$fatalities_p_w_regs_only) + ((dt_est_prov_reg$sumreach_regs_rhs2/(dt_est_prov_reg$sumreach_regs+dt_est_prov_reg$sumreach_regs_rhs2)) *
                                                                                       dt_est_prov_reg$fatalities_p_w_regs_rhs)
      
      dt_est_prov_reg$fatalities_p_m_regs <- ((dt_est_prov_reg$sumreach_regs/(dt_est_prov_reg$sumreach_regs + dt_est_prov_reg$sumreach_regs_rhs2)) *
                                            dt_est_prov_reg$fatalities_p_m_regs_only) + ((dt_est_prov_reg$sumreach_regs_rhs2/(dt_est_prov_reg$sumreach_regs+dt_est_prov_reg$sumreach_regs_rhs2)) *
                                                                                       dt_est_prov_reg$fatalities_p_m_regs_rhs)
      ...
      dt_est_prov_reg$fatalities_p_w_regs <- dt_est_prov_reg$fatalities_p_w_regs_only
      dt_est_prov_reg$fatalities_p_m_regs <- dt_est_prov_reg$fatalities_p_m_regs_only
    ...
    fatalities_p_w_country_only <- fatalities_p_m_country_only <- NA
    stillsick_p_w_country_only <- stillsick_p_m_country_only <- NA
    ...
    if(nrow(dt_country) != 0){
      ...
      fatalities_p_w_country_only <- ifelse(all(is.na(dt_country$fatalities)),
                                        NA, sum(dt_country$fatalities, na.rm = T)/sum(dt_country$reach[!is.na(dt_country$fatalities)]))
      fatalities_p_m_country_only <- ifelse(all(is.na(dt_country$fatalities)),
                                        NA, mean(dt_country$fatalities/dt_country$reach, na.rm = T))
      ...
      fatalities_p_w_country_only <- fatalities_p_m_country_only <- 0
      stillsick_p_w_country_only <- stillsick_p_m_country_only <- 0
    }
    ...
    dt_est_reg_count <- by(dt_est_prov_reg,
                           ...
                           function(x){
                             data.frame(countrycode = unique(x$countrycode),
                                        ...
                                        fatalities_p_w_regs = unique(x$fatalities_p_w_regs),
                                        fatalities_p_m_regs = unique(x$fatalities_p_m_regs),
...
#    dt_est_reg_count1 <- na.omit(dt_est_reg_count[,c(1,2,3,4,5,8)])
    dt_est_reg_count1 <- na.omit(dt_est_reg_count)
...
#    dt_est_reg_count2 <- na.omit(dt_est_reg_count[,c(1,2,3,6,7,8)])
    dt_est_reg_count2 <- na.omit(dt_est_reg_count)
    ...
    fatalities_p_w_country_rhs <- sum(weightcountry2 * dt_est_reg_count2$fatalities_p_w_regs)
    fatalities_p_m_country_rhs <- sum(weightcountry2 * dt_est_reg_count2$fatalities_p_m_regs)
    ...
    fatalities_p_w_counts <- sum(((sumreach_country/(sumreach_country + sumreach_country_rhs2)) * fatalities_p_w_country_only), 
                             ((sumreach_country_rhs2/(sumreach_country + sumreach_country_rhs2)) * fatalities_p_w_country_rhs), na.rm = T)
    fatalities_p_m_counts <- sum(((sumreach_country/(sumreach_country + sumreach_country_rhs2)) * fatalities_p_m_country_only),  
                             ((sumreach_country_rhs2/(sumreach_country + sumreach_country_rhs2)) * fatalities_p_m_country_rhs), na.rm = T)
    ...
    if (write_daily_file == T){
      dt_est_count <- data.frame(date=as.Date(j),
                                 ...
                                 fatalities_p_w_country_only = fatalities_p_w_country_only,
                                 ...
                                 fatalities_p_w_country_rhs = fatalities_p_w_country_rhs,
                                 ...
                                 fatalities_p_w_country =  fatalities_p_w_counts,
                                 stillsick_p_w_country =  stillsick_p_w_counts
      )
    ...
    df_aux <- dt_est_prov_reg[, c("date", "countrycode", "regioncode", "population_region", "p_w_regs", 
                                  "recent_p_w_regs", "fatalities_p_w_regs", "stillsick_p_w_regs")]
    ...
    df_aux %>% rename(country = countrycode, region = regioncode, 
                      ...
                      p_cases_fatalities = fatalities_p_w_regs,
    ...
    I_c_fatalities_p_w_country <- c(I_c_fatalities_p_w_country, fatalities_p_w_country_only)
    I_c_fatalities_p_m_country <- c(I_c_fatalities_p_m_country, fatalities_p_m_country_only)
    ...
    I_r_fatalities_p_w_country <- c(I_r_fatalities_p_w_country, fatalities_p_w_country_rhs)
    I_r_fatalities_p_m_country <- c(I_r_fatalities_p_m_country , fatalities_p_m_country_rhs)
    ...
    fatalities_p_w_country <- c(fatalities_p_w_country, fatalities_p_w_counts)
    fatalities_p_m_country <- c(fatalities_p_m_country, fatalities_p_m_counts)
  ...
  region_based_estimate <- data.frame(date = as.Date(dates),
                                      ...
                                      p_cases_fatalities = fatalities_p_w_country,
