				...
				for FIRSTCUTOFF in 2020-11-30; do
					...
					for CUTOFFINTERVAL in 1 ; do
						...
						for BASISDIM in 15 ; do
						    ...
						    Rscript runlag-$COUNTRY-pen$PEN$ALPHA-rmcl$RMCL$RMTH-smooth$SMOOTH$BASISDIM-lag$MILAG-$MXLAG-$SIGTOMATCH-$SIGTOTRY-$FIRSTCUTOFF-$LASTCUTOFF-$CUTOFFINTERVAL.R  > runlag-$COUNTRY-pen$PEN$ALPHA-rmcl$RMCL$RMTH-smooth$SMOOTH$BASISDIM-limrange$LIMRANGE-lag$MILAG-$MXLAG-$SIGTOMATCH-$SIGTOTRY-$FIRSTCUTOFF-$LASTCUTOFF-$CUTOFFINTERVAL.Rout 2>&1 &
						done
					    done 
					done
				    done
