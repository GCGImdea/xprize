...
do_smoothing <- function(country = "ES",
  ...
  if (nrow(dt) >= smooth_param) {
    ...
    try(
    dt_past <- smooth_column(dt_past, "p_cases", basis_dim = smooth_param, link_in ="log", monotone = T)
    , silent = F)
    
    cat("Smoothing p_cases_fatalities...\n")
    try(
    dt_past <- smooth_column(dt_past, "p_cases_fatalities", basis_dim = smooth_param, link_in ="log", monotone = T)
    , silent = F)

    cat("Smoothing p_cases_recent...\n")
    try(
    dt_past <- smooth_column(dt_past, "p_cases_recent", basis_dim = smooth_param, link_in ="log", monotone = F)
    , silent = F)
    
    cat("Smoothing p_cases_stillsick...\n")
    try(
    dt_past <- smooth_column(dt_past, "p_cases_stillsick", basis_dim = smooth_param, link_in ="log", monotone = F)
    , silent = F)
    ...
    write.csv(dt_past, paste0(estimates_path, "smooth/", country, "-estimate.csv"),
              row.names = FALSE)
  }
}
