...
estimates_path <- "../data/estimates-ccfr-fatalities/"
...
generate_estimates <- function(){
  country_codes <- list.files(ox_region_path, pattern="*.csv", full.names=FALSE)
  ...
  for (c in country_codes) {
    cat(paste0(ox_region_path, c, "-estimate.csv"))
    df <- read.csv(paste0(ox_region_path, c, "-estimate.csv"))
    plot_estimates(c, df$RegionCode[1], dts = df)
  }
